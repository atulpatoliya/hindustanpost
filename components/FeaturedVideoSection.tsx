...
export default function FeaturedVideoSection({ data }: { data?: Article[] }) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [activeVideoId, setActiveVideoId] = useState<string | null>(null);

  const [featured, ...rest] = data || [];
  const restItem = rest.slice(0, 4);
  ...
  const openHeroVideo = () => {
    setActiveVideoId(featured?.video?.length ? featured?.video[0] : "dQw4w9WgXcQ");
    setIsModalOpen(true);
  };
    ...
    <section className="mt-8">
      ...
      <div className=" grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 relative">
          <div
            className="relative border-2 border-yellow-500 cursor-pointer"
            onClick={openHeroVideo}
          >
            <div className="w-full h-96 relative">
              <Image
                src={featured?.featuredImage || "/fallback.png"}
                alt={featured?.title || "Featured item"}
                fill
