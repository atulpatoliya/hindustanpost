...
export default function WebStoryPage({ params }: { params: { slug: string } }) {
  ...
  async function fetchStories() {
    try {
      ...
      const filterBySlug = data.find((news) => news?.slug === params?.slug);
      setStories(filterBySlug || null);
  ...
  const slides = stories?.image;
  const totalSlides = stories?.image?.length || 0;
  ...
  function closeOverlay() {
    try {
      if (typeof window !== 'undefined') {
        const hasStory = new URLSearchParams(window.location.search).has(
          'story'
        );
        if (hasStory) {
          window.history.back();
    ...
    return (
      <div className="fixed inset-0 z-50 bg-[#8B4513] flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <Link href="/web-stories" className="underline">
            Go back to Web Stories
          </Link>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center md:p-0">
        {loadingStories ? (
          <div className="relative w-full h-full md:aspect-[9/16] md:w-[min(92vw,420px)] md:max-h-[90vh] md:rounded-[28px] overflow-hidden md:shadow-[0_24px_70px_rgba(0,0,0,0.55)] flex justify-center items-center">
            ...
            <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-20 w-[86%] px-4">
              <div className="mx-auto bg-white/90 backdrop-blur-xl rounded-[40px] px-7 py-6 shadow-[0_12px_50px_rgba(0,0,0,0.35)] border border-white/60">
                {!loadingStories ? (
                  ...
                  <div className="h-8 animate-pulse bg-gray-300 rounded w-full min-h-[170px]" />
                )}
              </div>
            </div>
                      ...
                      <span className="w-10 h-10 rounded bg-gray-200 flex items-center justify-center">
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth="2"
                            d="M13.172 7H7a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2v-6.172a2 2 0 00-.586-1.414l-3.828-3.828A2 2 0 0013.172 7z"
                          />
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth="2"
                            d="M7 7V5a2 2 0 012-2h6"
                          />
                        </svg>
                      </span>
